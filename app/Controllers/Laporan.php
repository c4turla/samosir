...
class Laporan extends BaseController
{
    ...
    $spreadsheet->setActiveSheetIndex(0)
                ...
                ->setCellValue('F1', 'Nama Nakhoda')
                ->setCellValue('G1', 'ABK')
                ->setCellValue('H1', 'Tanggal Masuk')
                ->setCellValue('I1', 'Tanggal Berangkat')
                ->setCellValue('J1', 'Nama Tangkahan')
                ->setCellValue('K1', 'Status')
                ->setCellValue('L1', 'Es')
                ->setCellValue('M1', 'Air')
                ->setCellValue('N1', 'Solar')
                ->setCellValue('O1', 'Oli')
                ->setCellValue('P1', 'Bensin')
                ->setCellValue('Q1', 'Lainnya')
    ...
    foreach($dataKeberangkatan as $data) {
        $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('F' . $column, $data->nama_nakhoda)
                    ->setCellValue('G' . $column, $data->abk)
                    ->setCellValue('H' . $column, $data->tanggal_masuk)
                    ->setCellValue('I' . $column, $data->tanggal_berangkat)
                    ->setCellValue('J' . $column, $data->nama_dermaga)
                    ->setCellValue('K' . $column, $data->status)
                    ->setCellValue('L' . $column, $data->es)
                    ->setCellValue('M' . $column, $data->air)
                    ->setCellValue('N' . $column, $data->solar)
                    ->setCellValue('O' . $column, $data->oli)
                    ->setCellValue('P' . $column, $data->bensin)
                    ->setCellValue('Q' . $column, $data->lainnya)
                    ->setCellValue('R' . $column, $data->keterangan);
        $column++;
    }
    // tulis dalam format .xlsx
    $writer = new Xlsx($spreadsheet);
