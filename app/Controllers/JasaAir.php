...
class JasaAir extends BaseController
{
    ...
    {
        $jasaAir = new JasaAirModel();
        $filename = date('y-m-d-H-i-s') . '-Order-Jasa-Air';
        $data = array(
            'jasaAir' => $jasaAir->find($id)
        );
        ...
        // load HTML content
        $dompdf->loadHtml(view('jasaair/cetakorder', $data));
    ...
    {
        $jasaAir = new JasaAirModel();
        $filename = date('y-m-d-H-i-s') . '-Kwitansi-Jasa-Air';
        $data = array(
            'jasaAir' => $jasaAir->find($id)
        );
        ...
        $dompdf->set_option('isHtml5ParserEnabled', true);
        // load HTML content
        $dompdf->loadHtml(view('jasaair/cetakperhitungan', $data));
